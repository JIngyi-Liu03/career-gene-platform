<template>
  <form class="auth-form" @submit.prevent="submit">
    ...
    <label class="field">
      <span>新密码</span>
      <input v-model="password" type="password" placeholder="请输入新密码（至少 6 位）" />
    </label>
    <label class="field">
      <span>确认新密码</span>
      <input v-model="confirm" type="password" placeholder="请再次输入新密码" />
    </label>
...
async function sendCode() {
  ...
  loading.value = true
  try {
    const r = await sendSmsCode(phone.value, 'reset')
